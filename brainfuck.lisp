...
(defun retrieve-right (code index amount)
    ;;(format *STANDARD-OUTPUT* "~a" amount)
    (if (= amount 0)
        index
        (let ((c (char code (+ index 1))))
            (if (char= c #\])
        ...
        (let ((c (char code iptr)))
            (cond
                ...
                ((char= c #\<)
                    (run-bf code (+ iptr 1) data (- dptr 1) len))
                ((char= c #\+)
                    (run-bf code (+ iptr 1) (replace-item data dptr (+ (index-lst data dptr) 1)) dptr len))
                ((char= c #\-)
                    (run-bf code (+ iptr 1) (replace-item data dptr (- (index-lst data dptr) 1)) dptr len))
                ((char= c #\.)
                    (format *STANDARD-OUTPUT* "~a" (code-char (index-lst data dptr)))
                    (run-bf code (+ iptr 1) data dptr len))
                ((char= c #\,)
                    (run-bf code (+ iptr 1) (replace-item data dptr (char-code (read-char))) dptr len))
                ((char= c #\[)
                    (if (= (index-lst data dptr) 0)
                        (run-bf code (retrieve-right code iptr 1) data dptr len)
                        (run-bf code (+ iptr 1) data dptr len)))
                ((char= c #\])
                    (if (not (= (index-lst data dptr) 0))
